...
#[pyfunction]
fn py_to_poly(input: String) -> PyResult<String> {
    Ok(execute(input))
}

/// A Python module implemented in Rust.
#[pymodule]
fn polydriver(_py: Python, m: &PyModule) -> PyResult<()> {
    m.add_function(wrap_pyfunction!(py_to_poly, m)?)?;
    Ok(())
}
...
use std::{path::Path, fs::{self, File}, process::{self}};
mod maincore;
